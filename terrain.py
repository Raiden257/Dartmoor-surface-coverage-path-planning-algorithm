...
if __name__=='__main__':
    _, ax = plt.subplots(subplot_kw={"projection": "3d"})
    tile = TerrainTile('so0122_dtm_2m.asc')
    tile.plot(ax=ax, show=False)
    
    
    # now test whole collection of tiles
    # tile_cltn = TerrainTileCollection('dataset\wales-dtm-2m_5699350') 
    ...
    # chol=0
    # rhow=0
    # for i in range(len(tile_cltn.tiles)):
    #     chol+=tile_cltn.tiles[i].ncols
    #     rhow+=tile_cltn.tiles[i].nrows
    # print(chol)
    # print(rhow)
    plt.show()
